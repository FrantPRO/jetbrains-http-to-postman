...
###`
	...
	// Check body - normalize JSON for comparison
	if item.Request.Body.Mode != "raw" {
		t.Errorf("Expected body mode 'raw', got '%s'", item.Request.Body.Mode)
	}

	// Parse both JSONs to compare structure instead of string formatting
  ...
  "email": "john@example.com"
	...
	err = json.Unmarshal([]byte(item.Request.Body.Raw), &actualJSON)
	if err != nil {
		t.Errorf("Failed to parse actual JSON: %v", err)
	}

	err = json.Unmarshal([]byte(expectedBody), &expectedJSON)
	if err != nil {
		t.Errorf("Failed to parse expected JSON: %v", err)
	}

	actualJSONBytes, _ := json.Marshal(actualJSON)
	expectedJSONBytes, _ := json.Marshal(expectedJSON)

	if string(actualJSONBytes) != string(expectedJSONBytes) {
		t.Errorf("JSON content mismatch.\nExpected: %s\nGot: %s", expectedJSONBytes, actualJSONBytes)
	}
